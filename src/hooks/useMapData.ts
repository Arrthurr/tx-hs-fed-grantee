...
import { HeadStartProgram, CongressionalDistrictFeature, LayerVisibility, CongressApiMember, CongressionalDistrict, CongressApiResponse, TxhsaRegion, TxhsaRegionName } from '../types/maps';
...
export const useMapData = () => {
  ...
  const [layerVisibility, setLayerVisibility] = useState<LayerVisibility>({
    ...
    districtBoundaries: false, // Legacy - removed in U8
  ...
  /**
   * Load the four TXHSA region geojson files in parallel.
   * Each file is validated; if any one fails to fetch or validate, the whole
   * load is reported as a regionsError so the UI doesn't render a partial
   * overlay (R4: regions must collectively cover Texas with no gaps).
   */
  const loadTxhsaRegions = useCallback(async () => {
    ...
    try {
      ...
      const results = await Promise.all(slugs.map(async slug => {
        ...
        // Region geojson files are single-feature FeatureCollections.
  ...
  useEffect(() => {
    ...
    loadCongressionalDistricts();
    loadTxhsaRegions();
  }, [loadHeadStartPrograms, loadCongressionalDistricts, loadTxhsaRegions]);
   ...
   */
  const isLoading = isLoadingPrograms || isLoadingDistricts || isLoadingCongressData || isLoadingRegions;

  /**
   * Check if there are any errors
   */
  const hasErrors = !!programsError || !!districtsError || !!congressDataError || !!regionsError;
  ...
  }, [programsError, districtsError, congressDataError, regionsError, loadHeadStartPrograms, loadCongressionalDistricts, loadCongressionalData, loadTxhsaRegions]);
