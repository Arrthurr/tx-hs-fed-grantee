# Texas Head Start Interactive Map - Codex Guide
...
│                 # (module, tags, problem_type); relevant when debugging or
│                 # implementing in documented areas
├── plans/        # implementation plans for past and in-flight work
└── brainstorms/  # upstream requirements / discovery docs
```
...
- `src/data/tdemCountyRegions.ts` (county → TDEM region lookup, 4-way merge mapping, and `txhsaCountyOverrides`)

`txhsaCountyOverrides` assigns specific counties directly to a TXHSA region, bypassing the default `tdemToTxhsaRegion` mapping. The county's TDEM number is left untouched — TDEM is a factual upstream source we don't rewrite. Use sparingly; each override is a deliberate, reviewable deviation.

Run `npm run build:regions` after updating any of these inputs to regenerate the four output files.
...
- **Gotcha**: An override in `txhsaCountyOverrides` that detaches a county from the contiguous body of its assigned region will produce a `MultiPolygon` for that region (e.g., overriding Shelby/Nacogdoches to East while Hardin/Tyler stay North isolates the pair). The runtime handles MultiPolygon fine; no action needed unless the disconnect is unintentional.
